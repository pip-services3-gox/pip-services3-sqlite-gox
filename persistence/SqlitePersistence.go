...
func (c *SqlitePersistence[T]) Open(ctx context.Context, correlationId string) (err error) {
	if c.opened {
		return nil
	}
	...
	if err != nil {
		c.Client = nil
		err = cerr.NewConnectionError(correlationId, "CONNECT_FAILED", "Connection to sqlite failed"+err.Error()).WithCause(err)
	} else {
		c.opened = true
		c.Logger.Debug(ctx, correlationId, "Connected to sqlite database %s, table %s", c.DatabaseName, c.QuotedTableName())
	}
...
//		- correlationId (optional) transaction id to trace execution through call chain.
//	Returns: error or nil no errors occurred.
func (c *SqlitePersistence[T]) Close(ctx context.Context, correlationId string) (err error) {
	if !c.opened {
		return nil
	}
...
func (c *SqlitePersistence[T]) CreateSchema(ctx context.Context, correlationId string) (err error) {
...
func (c *SqlitePersistence[T]) GetPageByFilter(ctx context.Context, correlationId string,
...
func (c *SqlitePersistence[T]) GetCountByFilter(ctx context.Context, correlationId string,
...
func (c *SqlitePersistence[T]) GetListByFilter(ctx context.Context, correlationId string,
...
func (c *SqlitePersistence[T]) GetOneRandom(ctx context.Context, correlationId string, filter string) (item T, err error) {
	count, err := c.GetCountByFilter(ctx, correlationId, filter)
	if err != nil {
		return item, err
	}
...
func (c *SqlitePersistence[T]) Create(ctx context.Context, correlationId string, item T) (result T, err error) {
...
func (c *SqlitePersistence[T]) DeleteByFilter(ctx context.Context, correlationId string, filter string) error {
