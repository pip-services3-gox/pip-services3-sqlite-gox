...
func (c *SqliteConnection) Open(ctx context.Context, correlationId string) error {
	...
	if err != nil {
		c.Logger.Error(ctx, correlationId, err, "Failed to resolve sqlite connection")
		return err
	}
	c.Logger.Debug(ctx, correlationId, "Connecting to sqlite")

	con, err := sql.Open("sqlite3", uri)

	if err != nil || con == nil {
		err = cerror.NewConnectionError(correlationId, "CONNECT_FAILED", "Connection to sqlite failed").WithCause(err)
		return err
	}
	c.Connection = con
	c.DatabaseName = uri
...
func (c *SqliteConnection) Close(ctx context.Context, correlationId string) error {
	...
	if err != nil {
		return cerror.NewConnectionError(correlationId, "DISCONNECT_FAILED", "Disconnect from sqlite failed: ").WithCause(err)
	}

	c.Logger.Debug(ctx, correlationId, "Disconnected from sqlite database %s", c.DatabaseName)
	c.Connection = nil
	c.DatabaseName = ""
